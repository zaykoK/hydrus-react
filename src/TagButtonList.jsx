...
function TagButtonList(props) {
    ...
    useEffect(() => {
      ...
      for (let t in props.tags) {
        tagSet.push(<button key={props.tags[t]} style={TagTools.getTagButtonStyle(sortedTags[t].namespace)} onClick={() => { props.removeTag(props.tags[t]); }}>{tagTupleToString(props.tags[t])}</button>);
      }
      setTagList(tagSet);
    }, []);
